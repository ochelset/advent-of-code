...
class SpaceImage:
	...
	def add(self, digit):
		if not self.layer:
			self.layer = SpaceImageLayer(self.width, self.height)

		self.layer.add(digit)
		if self.layer.isComplete:
			self.layers.append(self.layer)
			self.layer = None

	def render(self):
		print()
		output = self.layers[0].data[:]
		for layer in self.layers[::-1]:
			for pos, pixel in enumerate(layer.data):
				output[pos] = pixel if pixel != 2 else output[pos]

		for i in range(0, self.height):
			start = i * self.width
			line = []
			for pixel in output[start:start + self.width]:
				line.append(self.style if pixel == 1 else ' ')
...
class SpaceImageLayer:
	...
	def add(self, digit):
		self.data.append(digit)
		if not digit in self.histogram:
			self.histogram[digit] = 0
		self.histogram[digit] += 1

#
#

file = open("data/8.data", "r")
data = file.read()
file.close()
...
for index in range(length):
	...
	image.add(int(data[index]))

fewestZeroLayer = None
for layer in image.layers:
	if not fewestZeroLayer or layer.histogram[0] < fewestZeroLayer.histogram[0]:
		fewestZeroLayer = layer

print("Part 1:", fewestZeroLayer.histogram[1] * fewestZeroLayer.histogram[2])
print("Part 2:")
image.render()
