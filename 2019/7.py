...
When a copy of the program starts running on an amplifier, it will first use an input instruction to ask the amplifier for its current phase 
setting (an integer from 0 to 4). Each phase setting is used exactly once, but the Elves can't remember which amplifier needs which phase setting.
...
this phase setting sequence may not have been the best one; another sequence might have sent a higher signal to the thrusters.
...
the last output signal from amplifier E is sent to the thrusters. Your job is to find the largest output signal that can be sent to 
...
class AmplifierController:
	...
	def add(self, amplifier):
		self.amplifiers.append(amplifier)

	def applyPhaseSettings(self, phaseSettings=[]):
		for n, phaseSetting in enumerate(phaseSettings):
			self.amplifiers[n].setPhase(phaseSetting)

	def reset(self):
		for amplifier in self.amplifiers:
			amplifier.reset()

	def run(self, phaseSettings=[]):
		self.applyPhaseSettings(phaseSettings)
		self.reset()

		amplifier = self.amplifiers[0]
		amplifier.inputValue = 0

		while amplifier:
			if amplifier.controller.isFinished:
				break

			amplifier.controller.silent = True
			if amplifier.outputValue == None:
				amplifier.controller.inputValues = [amplifier.phase, amplifier.inputValue]
			else:
				amplifier.controller.inputValues = [amplifier.inputValue]

			amplifier.outputValue = amplifier.controller.execute()

			amplifier = amplifier.target
			if amplifier:
				amplifier.inputValue = amplifier.source.outputValue
...
class Amplifier:
	...
	def __repr__(self):
		return "<Amplifier %s [%s -> %s => %s]>" % (self.name, self.source.name if self.source else ' ', self.name, self.target.name if self.target else ' ')
...
if True:
	...
	for phase in phaseSettings:
		ACS.run(phase)
		if e.outputValue != None and e.outputValue > highest:
			highest = e.outputValue

	print("Part 1, Final output:", highest)
...
print("Part 2, Final output:", highest)
