...
class Point():
	...
	def __repr__(self):
		return "Point %s: <%d, %d> [%d]" % (self.name, self.x, self.y, self.steps)
	...
	def intersects(self, point) -> bool:
...
def move(point, instruction, board) -> Point:
	...
	for n in range(length):
		...
		if point.at in board:
			if board[point.at].name != point.name:
				board["intersections"].append(point)
			continue

		board[point.at] = point
...
for instruction in route1.split(','):
	l1 = move(l1, instruction, board)

for instruction in route2.split(','):
	l2 = move(l2, instruction, board)

shortest = None
fewest = None
for a in board["intersections"]:
	distance = a.distance(O)
	if shortest == None or shortest.distance(O) > distance:
		shortest = a

	steps = a.steps + board[a.at].steps
	if fewest == None or fewest > steps:
		fewest = steps

print("Shortest Manhattan distance:", shortest, shortest.distance(O))
print("Shortest walking distance:", fewest)