...
there are 10007 cards in the deck numbered 0 through 10006. The deck must be new - they're still in factory order, with 0 on the top,
...
this technique is carefully designed so it will never put two cards in the same position or leave a position empty.
...
for command in data.strip().split("\n"):
    ...
    if command == "deal into new stack":
        ...
        count = int(command.replace("deal with increment ", ""))
        stack = len(deck)*[None]
        index = 0
        lindex = 0
        while len(deck):
            card = deck.pop(0)
            stack[index % CARDS] = card
            index += count
        deck = stack[:]
    elif command.startswith("cut"):
        count = int(command.replace("cut ", ""))
        cut = []
        if count > 0:
            cut = deck[:count]
            deck = deck[count:]
