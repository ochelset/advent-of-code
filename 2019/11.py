...
and so any input instructions at this point should be provided 0. Suppose the robot eventually outputs 1 (paint white) and then 0 (turn left).
...
the robot painted 6 panels at least once. (It painted its starting panel twice,
...
a valid registration identifier is always eight capital letters. After starting the robot on a single white panel instead,
...
data = list(map(int, open("data/11.data").read().strip().split(',')))
...
robot = EmergencyHullPaintingRobot(data)
robot.start(panels)

print("Part 1:", len(robot.log))
