...
the missing tiles count as empty space.) This process happens in every location simultaneously; that is,
...
then left-to-right in the second row, and so on. Each of these tiles is worth biodiversity points equal to increasing
...
The tile in the middle of the grid is actually another 5x5 grid, the grid in your scan is contained as the middle tile of a larger 5x5 grid,
...
def epoq():
    ...
    for y in range(5):
        ...
        for x in range(5):
            ...
            infested = len(list(filter(lambda x: x == "#", adjacent)))

            if tile == ".":
                if infested == 1 or infested == 2:
                    tile = "#"
            elif tile == "#":
                if infested != 1:
                    tile = "."
...
def getAdjacent(x, y):
    ...
    south = [x, y+1]
    result = []
    for pos in (west, north, east, south):
        if pos[0] < 0 or pos[0] >= 5 or pos[1] < 0 or pos[1] >= 5:
            result.append(None)
        else:
            result.append(getAt(pos[0], pos[1]))
