...
def move_submarine(aiming: bool=False) -> int:
    pos = [0, 0, 0]
    ...
    for line in inputdata:
        ...
        if command == "forward":
            pos[0] += movement
            pos[1] += movement * pos[2] if aiming else 0
        elif command == "up":
            pos[1] -= movement if not aiming else 0
            pos[2] -= movement
        elif command == "down":
            pos[1] += movement if not aiming else 0
            pos[2] += movement
...
print("Part 1:", move_submarine())
print("Part 2:", move_submarine(aiming=True))
