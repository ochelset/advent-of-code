...
def scan(sweeping: bool = False) -> int:
    increases = 0
    start = 0 if not sweeping else 2
    prev = None
    for i in range(start, len(inputdata)):
        depth = int(inputdata[i]) if not sweeping else sum([int(inputdata[i-2]), int(inputdata[i-1]), int(inputdata[i])])
        if prev != None and depth > prev:
            increases += 1
        prev = depth
...
print("Part 1:", scan())
print("Part 2:", scan(sweeping=True))
