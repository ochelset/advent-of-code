...
inputdata = [int(x) for x in open("input.data").read().split(",")]

count_set = {}
least_fuel = { 1: 10000000000, 2: 10000000000 }
for target in range(1000):
    ...
    for pos in inputdata:
        ...
        dist_fuel = 0
        if distance in count_set:
            dist_fuel = count_set[distance]
        else:
            for i in range(distance+1):
                dist_fuel += i
            count_set[distance] = dist_fuel

        fuel1 += distance
        fuel2 += dist_fuel

    if fuel1 < least_fuel[1]:
        least_fuel[1] = fuel1
    if fuel2 < least_fuel[2]:
        least_fuel[2] = fuel2

print("Part 1:", least_fuel[1])
print("Part 2:", least_fuel[2])
