...
for line in inputdata:
    ...
    gift = {}
    possible = True
    possible_updated = True
    for thing in things.split(", "):
        name, value = thing.split(": ")
        gift[name] = int(value)

        if gift[name] != MFCSAM_RESULT[name]:
            possible = False

    for name in gift.keys():
        if name in { 'cats', 'trees' }:
            possible_updated = False if gift[name] <= MFCSAM_RESULT[name] else possible_updated
        elif name in { 'pomeranians', 'goldfish' }:
            possible_updated = False if gift[name] >= MFCSAM_RESULT[name] else possible_updated
        elif gift[name] != MFCSAM_RESULT[name]:
            possible_updated = False

    if possible:
        print("Part 1:", aunt, gift)

    if possible_updated:
        print("Part 2:", aunt, gift)
